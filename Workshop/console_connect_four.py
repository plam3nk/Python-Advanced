def matrix_creator(rows: int, columns: int):
    matrix = []
    for _ in range(rows):
...
def player_data_to_main_logic(playground, player_index, player):
    start_row_index = 0
...
def check_win(playground, row_index, column_index, win_counter):
    ...
    def check_up_left_win_condition(playground, row_index, column_index, min_row_index, min_column_index):
        diagonal_index = min(min_row_index - row_index, abs(min_column_index - column_index))
    ...
    # right, left, up, down
    right_win_condition = check_right_win_condition(playground, row_index, column_index, max_column_index)
    left_win_condition = check_left_win_condition(playground, row_index, column_index, min_column_index)
    up_win_condition = check_up_win_condition(playground, row_index, column_index, min_row_index)
    down_win_condition = check_down_win_condition(playground, row_index, column_index, max_row_index)

    # diagonals
    down_right_win_condition = check_down_right_win_condition(playground, row_index, column_index, max_row_index, max_column_index)
    down_left_win_condition = check_down_left_win_condition(playground, row_index, column_index, max_row_index, min_column_index)
    up_right_win_condition = check_up_right_win_condition(playground, row_index, column_index, min_row_index, max_column_index)
    up_left_win_condition = check_up_left_win_condition(playground, row_index, column_index, min_row_index,min_column_index)
    ...
    return any(len(current_list) == win_counter and len(set(current_list)) == 1 for current_list in winner_checker)


def gameplay(playground, win_counter):
    current_player, player_two = 1, 2

    while True:
        current_choice = player_choice(current_player)
        row_index, column_index = player_data_to_main_logic(playground, current_choice, current_player)
        print_func(playground)
        if check_win(playground, row_index, column_index, win_counter):
            print(f"Winner is: Player {current_player}!")
            break
        current_player, player_two = player_two, current_player
...
try:
    ...
    win_counter = int(input("Number of win combination: "))
    playground = matrix_creator(rows, columns)
